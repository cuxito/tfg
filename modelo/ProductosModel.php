...
class productosModel extends Conexion{
    ...
    public function getAll($categoria)
    {
        ...
        try {
            if($categoria == ""){
                $sql = "select id_producto, nombre_prod, cantidad_prod, stock, PVP, descuento, imagen from $this->table";
            }
            else{
                $sql = "select id_producto, nombre_prod, cantidad_prod, stock, PVP, descuento, imagen from $this->table where categoria = '$categoria'";
